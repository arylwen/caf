...
## Procedure (deterministic helper only)

1) Run:

- `node tools/caf/project_task_backlog_v1.mjs <name>`

Rules:
- Do **not** print the invocation.
- If it exits non-zero, STOP and surface the error or feedback packet path.
- Do **not** invent a replacement script under `tools/caf/`.

2) Run the shape/integrity check:

- `node tools/caf/backlog_integrity_check.mjs <name>`
