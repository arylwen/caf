...
"""Postgres-backed collection_permissions repository."""
...
from .models import CollectionPermissionsModel


class PostgresCollectionPermissionsRepository:
    def __init__(self, session_factory):
        self._session_factory = session_factory

    @staticmethod
    def _to_dict(row: CollectionPermissionsModel) -> dict:
        payload = deepcopy(row.payload or {})
        payload["collection_permission_id"] = row.collection_permission_id
    ...
    def list_items(self, *, tenant_id: str) -> list[dict]:
        with self._session_factory() as session:
            rows = session.scalars(
                select(CollectionPermissionsModel).where(CollectionPermissionsModel.tenant_id == tenant_id)
            ).all()
            return [self._to_dict(row) for row in rows]

    def create_item(self, *, tenant_id: str, actor_user_id: str, payload: dict) -> dict:
        now = datetime.now(UTC)
        row = CollectionPermissionsModel(
            collection_permission_id=str(uuid4()),
            tenant_id=tenant_id,
    ...
    def update_item(self, *, tenant_id: str, actor_user_id: str, item_id: str, payload: dict) -> dict:
        with self._session_factory() as session:
            row = session.scalar(
                select(CollectionPermissionsModel).where(
                    CollectionPermissionsModel.tenant_id == tenant_id,
                    CollectionPermissionsModel.collection_permission_id == item_id,
                )
            )
            if row is None:
                raise KeyError(f"collection_permissions {item_id} not found")
