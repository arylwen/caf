...
`node tools/caf/worker_traceability_mindmap_v3.mjs <instance_name>`

This script:
- emits `flowchart TD` with increased `rankSpacing`/`nodeSpacing` (diagram height usable in constrained renderers)
- adds explicit **Architecture pins** and **Technology pins / resolved atoms** subgraphs
- links pins/atoms to patterns using grounded sources (expectation table + candidate evidence)
