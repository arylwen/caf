...
### Deterministic generation procedure (instruction-only)
...
2) Compute:
   ...
   - `COMPANION_REPO_TARGET` = `companion_repo_target`

3) Create the target directory (and parents) if it does not exist.

4) Write seed files by copying templates and applying allowed substitutions.

   For each seed file path below:
   - If the target file does not exist: create it.
   - If the target file exists:
     - If overwrite=false: do not modify it.
     - If overwrite=true: overwrite it (replace the file contents).

   Seed files:
   - `AGENTS.md` from `skills/caf-companion-init/templates/AGENTS.md`
   - `README.md` from `skills/caf-companion-init/templates/README.md`

5) Do not create executable files.
