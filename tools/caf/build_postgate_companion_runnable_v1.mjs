...
export async function internal_main(argv = process.argv.slice(2)) {
  ...
  if (existsSync(resolvedPath)) {
    try {
      ...
      expectedDeploymentStackName = normalizeScalar(resolvedObj?.deployment?.stack_name) || instanceName;
    } catch {
      resolvedObj = null;
      databaseEngine = '';
    }
  ...
  if (assemblerCompleted && bindingContracts && bindingContracts.bindings.length > 0) {
    ...
    if (bindingIssues.length > 0) {
      const fp = await writeFeedbackPacket(
        ...
        [
          'Update the assembler task to close each declared interface binding in the composition root or equivalent assembly boundary.',
    ...
    if (fallbackIssues.length > 0) {
      const fp = await writeFeedbackPacket(
        ...
        [
          ...
          'Make repository_factory (or equivalent composition root) fail closed when DATABASE_URL is missing/empty or does not match the resolved engine.',
