...
| Architecture Binding Pattern (ABP) | A plane-neutral architecture-style artifact that defines structural roles, dependency direction, and style invariants for implementation shape. | Clean Architecture, Layered Architecture, and similar architecture styles. | Plane placement, cross-plane contracts, technology selection, or product/domain rationale. |
...
| Core Problem Pattern (CPP) | A reusable solution to a recurring architectural problem or invariant at the problem/semantics level. | Persistence, policy, context propagation, validation, observability. | Technology choice, plane placement, or architecture style. |
| Plane Binding Pattern (PBP) | A plane-scoped binding artifact that maps selected ABP roles to plane-local materialization surfaces and path conventions. | Plane-local role placement for CP, AP, DP, AI, or ST. | Architecture style selection or technology realization. |
...
| Technology Binding Pattern (TBP) | A technology-specific binding artifact that realizes already-selected style and plane choices in concrete frameworks, runtimes, and conventions. | Framework/runtime/ORM/container/service wiring choices. | Core problem semantics, architecture style selection, or plane placement. |
