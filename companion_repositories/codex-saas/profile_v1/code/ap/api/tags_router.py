# CAF_TRACE: generated_by=Contura Architecture Framework (CAF)
# CAF_TRACE: task_id=TG-20-api-boundary-tags
# CAF_TRACE: capability=api_boundary_implementation
# CAF_TRACE: instance=codex-saas

"""API boundary transport adapter for tags resources."""

from typing import Any

from fastapi import APIRouter, Depends, HTTPException, Request, status

from ..application.services import PolicyFacade, TagsFacade
from .auth_context import resolve_auth_context
from .dependencies import policy_dependency, tags_dependency

router = APIRouter(prefix="/ap/tags", tags=["tags"])


def _authorize(
    *,
    request: Request,
    policy: PolicyFacade,
    action: str,
) -> tuple[str, str]:
    claims = resolve_auth_context(request.headers)
    decision = policy.evaluate(
        action=action,
        tenant_id=claims.tenant_id,
        principal_id=claims.principal_id,
        policy_version=claims.policy_version,
    )
    if not decision.allowed:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=decision.reason)
    return claims.tenant_id, claims.principal_id


@router.get("")
def list_tags(
    request: Request,
    policy: PolicyFacade = Depends(policy_dependency),
    facade: TagsFacade = Depends(tags_dependency),
) -> dict[str, Any]:
    tenant_id, _ = _authorize(request=request, policy=policy, action="tags.list")
    return {"items": facade.list_items(tenant_id=tenant_id)}


@router.post("", status_code=status.HTTP_201_CREATED)
def create_tag(
    payload: dict[str, Any],
    request: Request,
    policy: PolicyFacade = Depends(policy_dependency),
    facade: TagsFacade = Depends(tags_dependency),
) -> dict[str, Any]:
    tenant_id, principal_id = _authorize(request=request, policy=policy, action="tags.create")
    payload_copy = dict(payload)
    payload_copy.pop("tenant_id", None)
    payload_copy.pop("tag_id", None)
    return facade.create_item(
        tenant_id=tenant_id,
        actor_user_id=principal_id,
        payload=payload_copy,
    )

