...
- Lifecycle and safety posture:
  - All architecture outputs in this phase are design-time scaffolds only because `lifecycle.generation_phase=architecture_scaffolding` and `runnable_code_approved=false`.
...
- Runtime and packaging guardrails:
  ...
  - Infrastructure target is local `docker_compose`; design artifacts must remain compatible with local multi-service composition.
  - Control Plane and Application Plane runtime shape remains `api_service_http` for this phase (`planes.cp.runtime_shape`, `planes.ap.runtime_shape`).
- Evidence and traceability:
  - Decision, policy, safety-gate, and data-access outcomes must emit inline evidence synchronously (`AI-6`, `AP-6`, `DP-5`, `ST-6`).
  - Governance surfaces (policy authoring, safety gate definitions, lifecycle ownership) remain centralized in the control plane (`CP-2`, `CP-4`, `CP-5`, `CP-6`).
...
(Deterministic summary of resolved guardrails/posture from `guardrails/profile_parameters_resolved.yaml` and policy-aligned pin intent. No new architecture choices.)
...
- Allowed write scope is constrained to the instance companion profile and the reference architecture playbook/guardrails/meta/layer_7/feedback paths for `codex-saas`.
- Forbidden actions explicitly block production claims, runnable production code generation, deployment, new architecture choice invention, and edits outside allowed paths.
...
### M-7: CAF-COMP-01  -  Evidence Generation & Traceability (confidence: medium)
**Plane:** both
**Evidence:**
...
### M-10: CAF-XPLANE-01  -  Allowed Cross-Plane Interaction Patterns (confidence: medium)
**Plane:** both
**Evidence:**
...
### M-12: CAF-IAM-GOV-04  -  Identity Governance Separation for Policy Runtime (confidence: medium)
**Plane:** control
**Evidence:**
...
### M-13: CAF-COMP-02  -  Evidence Schema Consistency Across Planes (confidence: medium)
**Plane:** both
**Evidence:**
...
### M-14: CAF-COH-02  -  Canonical Term Registry for Cross-Plane Contracts (confidence: medium)
**Plane:** both
**Evidence:**
...
### M-15: CAF-MTEN-AGOBS-01  -  Tenant-Scoped Auditability for Multi-Tenant Operations (confidence: medium)
**Plane:** both
**Evidence:**
