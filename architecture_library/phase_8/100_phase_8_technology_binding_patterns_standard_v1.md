...
Technology Binding Patterns (TBPs) are Phase 8 artifacts that **bind stack-independent core patterns** (Layer 1) to **concrete technology realizations** (files, modules, configuration, and naming conventions).
...
- `layout.role_bindings.*.evidence_contains` / `evidence_not_contains` (when present) are **hints only**.
  They are not compiled into deterministic filesystem checks.
...
- **Gates**: *semantic* acceptance criteria that the planner compiles into Task Graph:
