#!/usr/bin/env node
/**
 * CAF implementation scaffolding reset helper (v1)
 *
 * Purpose:
 ...
 *
 * Usage:
 *   node tools/caf/implementation_scaffolding_reset_v1.mjs <instance_name> overwrite
 */
...
async function writeFeedbackPacket(layout, instanceName, slug, observedConstraint, minimalFixLines, evidenceLines) {
  ...
  const body = [
    `# Feedback Packet - caf implementation scaffolding reset`,
    '',
    `- Date: ${nowDateYYYYMMDD()}`,
    cafBulletStampLine(),
    `- Instance: ${instanceName}`,
    `- Stuck At: tools/caf/implementation_scaffolding_reset_v1.mjs`,
...
async function main() {
  ...
  if (args.length < 1) {
    die('Usage: node tools/caf/implementation_scaffolding_reset_v1.mjs <instance_name> overwrite', 2);
  }
  ...
  if (!overwrite) {
    die('Refusing to reset without explicit overwrite. Usage: node tools/caf/implementation_scaffolding_reset_v1.mjs <instance_name> overwrite', 6);
  }
